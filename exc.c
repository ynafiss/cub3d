...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: Invalid date        by ynafiss           #+#    #+#             */
/*   Updated: 2023/07/24 23:12:43 by ynafiss          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move_dir(char **map, t_mlx *p, int x, int y)
{
	...
	if(p->up == 1)
	{
			if (map[(int)((p->y_p ) / 50)][(int)((p->x_p + cos(p->angle) * 20) / 50)] != '1')
				p->x_p  = p->x_p + cos(p->angle) * 4;
			if (map[(int)((p->y_p - sin(p->angle) * 20) / 50)][(int)((p->x_p) / 50)] != '1')
	...
	{
			if (map[(int)((p->y_p) / 50)][(int)((p->x_p + cos(p->angle + M_PI/2) * 20) / 50)] != '1' )
				p->x_p  = p->x_p + cos(p->angle + M_PI/2) * 4;
			if (map[(int)((p->y_p - sin(p->angle + M_PI/2) * 20) / 50)][(int)(p->x_p / 50)] != '1')
